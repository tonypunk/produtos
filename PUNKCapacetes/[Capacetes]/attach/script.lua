	...
	pAttach = {
		...
		refreshRender = function(self)
			...
			for ped in pairs(self.inStreamPeds) do
				if self.pedInstances[ped] then
					if self.pedInstances[ped].list then
			...
			if len > 0 and not self.pedsProcessedAdded then
				--addEventHandler("onClientPreRender", root, self.onPedsProcessed)
			...
			elseif len == 0 and self.pedsProcessedAdded then
				--removeEventHandler("onClientPreRender", root, self.onPedsProcessed)
		...
		onPedsProcessed = function()
			...
			for i = 1, #pAttach.preparedToRenderInstances do
				...
				if isElement (ped) and isOnScreen(ped) then
					...
					if not bMCache[boneid] then
						...
						bMCache[boneid] = boneMat
							--[[
						local d = getDetails (ped)
						local p = d[2]
							if p then
								if getElementType (p) == "ped" or getElementType (p) == "player" then
								boneMat = getBoneMatrix(p, d[3])
								bMCache[boneid] = boneMat
								end
							else
							boneMat = getElementMatrix(ped)
...
--bone
	...
	addEvent ("pAttach:attach", true)
	addEventHandler ("pAttach:attach", getRootElement(), attach)
