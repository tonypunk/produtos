...
function mapIni ()
	...
	if localPlayer then
	txd = engineLoadTXD ("files/models/ammu_nation_ls.txd", 4552)
	engineImportTXD(txd, 4552)
	dff = engineLoadDFF ("files/models/ammu_nation_ls.dff", 4552)
	engineReplaceModel(dff, 4552, true)
	col = engineLoadCOL("files/models/ammu_nation_ls.col")
...
for arma, tabela in pairs (properties) do
	if tabela.upgrades then
	local t = tabela.upgrades
		if t then
			for i=1, #t do
			local tab = t[i]
				if tab['tex'] then
					if tab['tex'].file then
					tab['texture'] = dxCreateTexture(tab['tex'].file)
					end
				end
			end
		end
...
shaders = {}
	for i=1, #prateleiras do
		...
		local arma = prateleiras[i][k].name
			if prateleiras[i][k].upgrades then
			local upgrades = prateleiras[i][k].upgrades
				if upgrades then
					for j=1, #upgrades do
					local tab = upgrades[j]
						if tab['tex'] then
		...
		local arma = prateleiras[i][k].name
			...
			local upgrades = prateleiras[i][k].upgrades
				if upgrades then
					for j=1, #upgrades do
					local tab = upgrades[j]
						if upgrade == tab.name then return tab.stats end
					end
				end
			end
		end
...
shaderID = {}
	function applyWeaponUpgrade (weapon, upgrade, value)
	local nome = 'Pistol'--weaponname[weapon] or exports['PUNK+Armas']:getWeaponName (weapon)
		if shaders[nome] then
			...
			local tab = shaders[nome][upgrade]
				...
				local shader = tab[i].element
					...
					local s = getElementsByType ('shader', resourceRoot)
						for i=1, #s do
						engineRemoveShaderFromWorldTexture (s[i], tex, weapon)
						end
					end
				end
			end
		...
		if exports['PUNK+Armas']:getDummy (weapon) then
		local dum = exports['PUNK+Armas']:getDummy (weapon)
			if isElement (dum) then
			local stats = getUpgradeStats (upgrade)
				for i=1, #stats do
				local stat = stats[i].stat
				local value = stats[i].value
...
--local p = createPed (179, x+0.5,y,z, 90)
local j = getElementsByType ('player')[1]
local x,y,z = getElementPosition (j)
--local p = createPed (179, x,y,z, -90)
--local p = createPed (179, x+1,y,z, 90)
end
...
weaponname = {}
	...
	local t = prateleiras[i]
		...
		local d = exports['PUNK+Armas']:getData()[arma]
			...
			local ob = exports['PUNK+Objetos']:createObject (d.model, x,y,z, rx,ry,rz)
...
--Eventos
addEvent ("PUNKArmasCarregar", true)
...
counter = 0
	...
	function loja ()
		...
		counter = 0
			--[[
			for i=1, #prateleiras do
			local tabela = armamentos[i]
			local arma = armamentos[i].name
			icone = false
				if counter == 0 then
				counter = counter + 1
					--if table.contain (objetos, name) then
					--icone = "accept4"
					--end
				else
					--if table.contain (objetos, name) then
			...
			addNativeButton(arma, tocolor (255,255,255), icone, "$"..tabela.price)
			end
			]]
		--Setup
		local tabela = prateleiras[1][21] or prateleiras[1][1]
			if table.contain (minhasArmas, tabela.name) then
	...
	function comprar (id, text)
		...
		if table.contain (minhasArmas, text) then
		destroyNativeUI ()
		createNativeUI(" ", text..' Upgrades',  'assets/24.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
		...
		idBotao = addNativeButton('Rounds x '..rounds, tocolor (255,255,255), false, tabela.rounds)
			for i=1, #prateleiras do
				for k=1, #prateleiras[i] do
					if text == prateleiras[i][k].name then
					local t = prateleiras[i][k].upgrades
						if t then
							for i=1, #t do
							addNativeButton(t[i].name, tocolor (255,255,255), false, t[i].price)
							end
						end
					break
					end
		...
		local preco = getNativeButtonPrice (id)
			if getPlayerMoney (localPlayer) >= preco then
				if active then
					...
					createHeadsupMessage (tab, "right")
					triggerServerEvent ("PUNKArmasComprar", localPlayer, text, preco)
					else
					local tab = {
					"Você adquiriu:",
					text,
					...
					setNativeButtonIcon (id, 'shop_gunclub_icon_b')
						...
						local nome, dummy, objeto = exports['PUNK+Armas']:getPedWeapon (jogador)
						applyWeaponUpgrade (objeto, text)
					--triggerServerEvent ("PUNKArmasComprar", localPlayer, text, preco)
					end
				else
				...
				createHeadsupMessage (tab, "right")
				triggerServerEvent ("PUNKArmasComprar", localPlayer, name, preco)
				setNativeButtonIcon (id, "shop_gunclub_icon_b")
				table.insert (minhasArmas, name)
				end
			else
			local tab = {
...
local marker = createMarker (1379.9541015625, -1282.4208984375, 13.546875, "cylinder", 1, 170,0,0,0)
	...
	function comprar (arma, preco)
	local jogador = source
		if getPlayerMoney (jogador) >= preco then
		local data = getData ()
		takePlayerMoney (jogador, preco)
			if data[arma] then
			exports['PUNK+Armas']:giveWeapon (jogador, arma, data[arma].clip, true)
			end
		end
	end
