...
local tab = {}
	if upgrades[arma] then
	local t = upgrades[arma]
		for i=1, #t do
	...
	if minhasArmas[arma] then
		if minhasArmas[arma].upgrades then
			for upgrade, valor in pairs (minhasArmas[arma].upgrades) do
				if upgrade == u then return true end
			end
		end
	end
...
function renderNative()
	...
	local pos = Vector2(window.titlepos["x"], window.titlepos["y"]+window.titlesize["y"]+(window.titlesize["y"]*(indexx+1)))
	dxDrawImage(pos, window.titlesize["x"], window.titlesize["y"]*10,weapon_stats)
	
	local size = {}
	local arma = armaAtual
	
	
	local statsPos = {
		['damage'] = {window.titlepos["x"], 7.475, window.titlesize["x"], window.titlesize["y"]*0.5},
		['rate'] = {window.titlepos["x"], 9.525, window.titlesize["x"], window.titlesize["y"]*0.5},
		['accuracy'] = {window.titlepos["x"], 11.97, window.titlesize["x"], window.titlesize["y"]*0.5},
		['weapon_range'] = {window.titlepos["x"], 14.435, window.titlesize["x"], window.titlesize["y"]*0.5},
	
	}
	
	for stat, tabela in pairs (statsPos) do
	...
	local px,py, sx,sy = unpack (statsPos[stat])
	local tamanho = (window.titlepos["x"]*41.75 - (window.titlepos["x"]*2.9) )
	dxDrawRectangle(px*2.925, (window.titlesize["y"]*py)+(window.titlesize["y"]*(indexx-1)), tamanho*size[stat], window.titlesize["y"]*0.5, tocolor(255,255,255,255), false)
		...
		local upgrade = currentUpgrade
			...
			local stats = getUpgradeStats (arma, upgrade)
				for s, value in pairs (stats) do
					...
					local tamanhoTotal = (window.titlepos["x"]*41.75 - (window.titlepos["x"]*2.9) ) * size3
						if size3 > 0 then
						cor = tocolor(0,0,255,255)
						else
						cor = tocolor(255,0,0,255)
						end
					dxDrawRectangle( (px*2.925) + (tamanho*size[stat]), (window.titlesize["y"]*py)+(window.titlesize["y"]*(indexx-1)), tamanhoTotal, window.titlesize["y"]*0.5, cor, false)
					end
				end
			end
...
local arma = currentWeapon
	...
	local u = getWeaponUpgrades (arma)
		for j=1, #u do
		local upgrade = u[j]
			if upgrade and net == upgrade then
				if currentUpgrade and hasUpgrade (arma, currentUpgrade) == false then
...
addEventHandler ('PUNKArmasComprar', getRootElement(), shopStart)
	...
	counter = 0
		...
		function loja ()
			...
			aberta = true
			destroyNativeUI ()
			createNativeUI(" ", "Pressione 'enter' para comprar",  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
			counter = 0
			--Setup
			...
			idBotao = addNativeButton(tabela.name, tocolor (255,255,255), icone, tabela.price)
			local px,py,pz = unpack (tabela['position'].active)
			local rx1,ry1,rz1 = unpack (tabela['rotation'].active)
			local ob = tabela.object
			objeto = ob
			armaAtual = tabela.name
			currentWeapon = armaAtual
			...
			--addEventHandler ("onClientChangeButton", getRootElement(), mudar
				...
				if minhasArmas then
					if minhasArmas[armaAtual] then
						if minhasArmas[armaAtual].upgrades then
							for item, valor in pairs (minhasArmas[armaAtual].upgrades) do
				...
				if estado then
					...
					elseif botao == "escape" or botao == "backspace" then
						...
						cancelEvent ()
							if active == true then
							destroyNativeUI ()
							createNativeUI(" ", "Pressione 'enter' para comprar",  'assets/24.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
								for i=1, #prateleiras do
		...
		tt = nil
			...
			armaAtual = tabela.name
				if minhasArmas[tabela.name] then
				setNativeButtonIcon (idBotao, 'shop_gunclub_icon_a')
				else
		...
		function comprar (id, text)
			...
			destroyNativeUI ()
			createNativeUI(" ", text..' Upgrades',  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
				
			local data = getData ()
				if data[text] then
				rounds = data[text].clip
				end
			idBotao = addNativeButton('Rounds x '..rounds, tocolor (255,255,255), false, tabela.rounds)
			local t = upgrades[text]
				if t then
					for i=1, #t do
					local tempButton = addNativeButton(t[i].name, tocolor (255,255,255), false, t[i].price)
						if minhasArmas[text].upgrades then
						local tab = t[i].name
							if minhasArmas[text].upgrades[tab] then
			...
			local preco = getNativeButtonPrice (id)
				if getPlayerMoney (localPlayer) >= preco then
					if active then
						if string.find (text, 'Rounds') then
						local tab = {
						"Você adquiriu munição:",
						text,
						"Preço: $"..preco,
						}
						createHeadsupMessage (tab, "right")
						local data = getData ()
						triggerServerEvent ("PUNKArmasComprar", localPlayer, localPlayer, currentWeapon, preco)
						else
						local tab = {
						"Você adquiriu:",
						text,
						...
						local tab = upgrades[currentWeapon]
							for i=1, #tab do
	...
	local marker = createMarker (1379.9541015625, -1282.4208984375, 13.546875, "cylinder", 1, 170,0,0,0)
		...
		function comprar (jogador, arma, preco)
			if getPlayerMoney (jogador) >= preco then
			local data = getData ()
				if data then
				local rounds = data[arma].clip
...
shaders = {}
	...
	if upgrades then
		for arma, tabela in pairs (upgrades) do
			for j=1, #tabela do
			local tab = tabela[j]
				if tab['tex'] then
	...
	function removeUpgradeFromPlayer(jogador, weapon, upgrade)
		if minhasArmas[jogador] then
			if minhasArmas[jogador][weapon] then
				if not minhasArmas[jogador][weapon].upgrades then minhasArmas[jogador][weapon].upgrades = {} end
				if not minhasArmas[jogador][weapon].upgrades[upgrade] then minhasArmas[jogador][weapon].upgrades[upgrade] = nil end
			end
		end
		if getPedWeapon(jogador) == weapon then
			if not localPlayer then
	...
	addEvent ('removeUpgradeFromPlayer', true)
	addEventHandler ('removeUpgradeFromPlayer', getRootElement(), removeUpgradeFromPlayer)
...
shaderID = {}
	...
	local dum = exports['PUNK+Armas']:getDummy (weapon)
		if isElement (dum) then
		outputChatBox (1)
		local stats = getUpgradeStats (nome, upgrade)
			for stat, value in pairs (stats) do
			outputChatBox (stat..': '..value)
			local setou = exports['PUNK+Armas']:setCustomWeaponProperty (dum, stat, value)
