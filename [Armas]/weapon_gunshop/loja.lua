...
function getWeaponUpgrades (arma)
local tab = {}
	if weapons[arma] then
		if weapons[arma].upgrades then
		local t = weapons[arma].upgrades
			for i=1, #t do
			table.insert (tab, t[i].name)
			end
		end
	end
return tab
...
function hasUpgrade (arma, u)
	--if string.find (u, 'Rounds') then return false end
	if minhasArmas[arma] then
		if minhasArmas[arma].upgrades then
			for upgrade, valor in pairs (minhasArmas[arma].upgrades) do
				if upgrade == u then return valor end
			end
		end
	end
...
function hasPlayerUpgrade (jogador, arma, u)
	--if string.find (u, 'Rounds') then return false end
	if minhasArmas[jogador][arma] then
		if minhasArmas[jogador][arma].upgrades then
			for upgrade, valor in pairs (minhasArmas[jogador][arma].upgrades) do
				if upgrade == u then return valor end
			end
		end
	end
...
function getUpgradeStats (arma, upgrade)
	if weapons[arma] and weapons[arma].upgrades then
	local tab = weapons[arma].upgrades
		for i=1, #tab do
			if tab[i].name == upgrade then
				if tab[i].stats then return tab[i].stats end
			end
		end
	end
end	

function getUpgradeStat (arma, upgrade, stat)
	if weapons[arma].upgrades then
	local tab = weapons[arma].upgrades
		for i=1, #tab do
			if tab[i].name == upgrade then
				for s, valor in pairs (tab[i].stats) do
					if s == stat then return valor end
				end
			end
		end
	end
...
function getUpgradeDescription (arma, upgrade)
	if weapons[arma].upgrades then
	local tab = weapons[arma].upgrades
		for i=1, #tab do
			if tab[i].name == upgrade then
				if upgrade == 'Extended Clip' then return {description[upgrade][1]..' '..arma..'.'} end
				if tab[i].description then
				return tab[i].description
				else
					if description[upgrade] then return description[upgrade] end
				end
			end
		end
	end
...
local weapon_stats = dxCreateTexture ('assets/weapon_stats.png', 'dxt3')
...
local data = getData ()
	for arma, tabela in pairs (data) do
		if type (arma) == 'string' then
			for atributo, valor in pairs (max) do
				if tabela[atributo] then
					if tabela[atributo] > max[atributo] then
					max[atributo] = tabela[atributo]
						if atributo == 'damage' then
							if tabela.fragments then
							max[atributo] = max[atributo]*tabela.fragments
							end
						end
					end
				local upgrades = getWeaponUpgrades (arma)
					for j=1, #upgrades do
					local upgrade = upgrades[j]
					local stats = getUpgradeStats (arma, upgrade)
						for stat, value in pairs (stats) do
							if atributo == stat then
								if value > max[atributo] then max[atributo] = value end
							end
						end
					end
				end
...
function updateRenderStats()
	...
	dxDrawImage(0,0, ax,ay, weapon_stats)
	
	local size = {}
	local arma = currentWeapon
	
	local statsPos = {
		['Damage'] = {ax - (ax*0.5), ay - (ay*0.825) },
		['Rate'] = {ax - (ax*0.5), ay - (ay*0.625) },
		['Accuracy'] = {ax - (ax*0.5), ay - (ay*0.425) },
		['Range'] = {ax - (ax*0.5), ay - (ay*0.2125) },
	
	}
	
local sy = (ay*0.0525)*0.75
local tamanho = (ax*0.9025)*0.5
	for s, tab in pairs (statsPos) do
	local stat = string.lower (s)
		if stat == 'range' then
		stat = 'weapon_range'
		end
	local atual = getWeaponProperty (arma, stat)
	local px = tab[1]
	local py = tab[2]
	local upgrades = getWeaponUpgrades (arma)
		for j=1, #upgrades do
		local upgrade = upgrades[j]
			if hasUpgrade (arma, upgrade) == true then
			local stats = getUpgradeStats (arma, upgrade)
		...
		if stat == 'damage' then
		local frag = getWeaponProperty (arma, 'fragments')
		size[stat] = (atual*frag)/max[stat]
		else
		size[stat] = atual/max[stat]
		end
	dxDrawRectangle(px, py, tamanho, sy, tocolor(100,100,100,255), false)
		if stat ~= 'rate' then
		dxDrawRectangle(px, py, tamanho*size[stat], sy, tocolor(255,255,255,255), false)
		else
		dxDrawRectangle(px, py, tamanho - (tamanho*size[stat]), sy, tocolor(255,255,255,255), false)
		end
	...
	local upgrade = currentUpgrade
		...
		local stats = getUpgradeStats (arma, upgrade)
			for s, value in pairs (stats) do
				if stat == s and size[stat] then
					if value < 0 then value = 0 end
				local size2 = value/max[stat]
				local size3 = size2 - size[stat]
				local tamanhoTotal = tamanho * size3
					if stat ~= 'rate' then
						if size3 > 0 then
						cor = tocolor(0,0,255,255)
						else
						cor = tocolor(255,0,0,255)
						end
					dxDrawRectangle(px + (tamanho*size[stat]), py, tamanhoTotal, sy, cor, false)
					else
						if size3 > 0 then
						cor = tocolor(255,0,0,255)
						else
					...
					dxDrawRectangle(px + (tamanho - (tamanho*size[stat])), py, -tamanhoTotal, sy, cor, false)
					end
				end
			end
		end
	...
	local size = tamanho*0.20
	for i=1, 4 do
	local size = tamanho*0.20
	
	dxDrawRectangle(ax - (ax*0.50725)+(size*i), 0, tamanho*0.0125, ay, tocolor(0,0,0,255), false)
	end
...
local renderPurchase = dxCreateRenderTarget(ax, ay, true)
function updateRenderPurchase()
    dxSetRenderTarget(renderPurchase, true)
    dxSetBlendMode("modulate_add")
	dxDrawRectangle (0,ay - (ay*0.8), ax, ay*0.25, tocolor (0,0,0,125))
	dxDrawBorderedText (1, "Purchased", 0, ay - (ay*0.95), ax, screenY - (screenY * 0.5), tocolor (98, 146, 99, 255), 3, "pricedown", "center", "center", true, false)
	local img = exports['weapon_replace']:getWeaponIcons ()
	local img = img[currentWeapon]
	dxDrawImage (ax - (ax*0.5995), ay - (ay*0.75), ax - (ax*0.8), ay - (ay*0.8), img)
	dxDrawBorderedText (1, currentWeapon, ax - (ax), ay - (ay*0.675), ax, screenY - (screenY * 0.5), tocolor (255,255,255, 255), 3, "default", "center", "center", true, false)
	
	dxSetBlendMode("blend")
    dxSetRenderTarget()
end
--addEventHandler ('onClientRender', root, updateRenderPurchase)
...
function renderNative()
    ...
    if window.name then
		...
		dxDrawImage (0,0,ax,ay, renderPurchase)
		end
	...
	indexx = i
		...
		if currentUpgrade then
			if hasUpgrade (currentWeapon, currentUpgrade) == true then
			tab = {'Item owned and equipped.'}
			elseif hasUpgrade (currentWeapon, currentUpgrade) == false then
			tab = {'Item owned but not equipped.'}
			else
			tab = getUpgradeDescription (currentWeapon, currentUpgrade)
			end
		else
		tab = weapons[currentWeapon] and weapons[currentWeapon].description
		end
		if tempDescription then tab = tempDescription end
		
		if tab then
			for i=1, #tab do
			local color = tocolor(0, 0, 0, 150)
			local textcolor = tocolor (255,255,255,255)
			local pos = Vector2(window.titlepos["x"], window.titlepos["y"]*1.05+window.titlesize["y"]+(window.titlesize["y"]*(indexx)))
			dxDrawRectangle(pos, window.titlesize, color)
				indexx = indexx +1
			local pos = Vector2(window.titlepos["x"]+(15/zoom), window.titlepos["y"]*1.05+window.titlesize["y"]/2+(window.titlesize["y"]*(indexx)))
			dxDrawText(tab[i], pos, nil, nil, textcolor, 1, window.titlefont or "default-bold", "left", "center")
			end
		end
		
		
	
	local pos = Vector2(window.titlepos["x"], window.titlepos["y"]*1.1+window.titlesize["y"]+(window.titlesize["y"]*(indexx)))
	dxDrawImage(pos, window.titlesize["x"], window.titlesize["y"]*5,renderStats)
...
function setNativeSwitchSelection(id,selection)
    ...
    assert(tonumber(id),"Bad argument @ setNativeSwitchSelection [expected number at argument 1,  got "..type(id).." '"..id.."'']")
...
function addNativeButton(text, color, icon, price)
    ...
    local table = {
        ...
        ["text"] = text,
        ["icon"] = icon or false,
		["price"] = price and price or false,
    }
...
function setNativeButtonName (id, text) 
 if not id or not text then return end
    assert(tonumber(id),"Bad argument @ setNativeButtonName [expected number at argument 1,  got "..type(id).." '"..id.."'']")
    assert(text ~= "box","Invalid icon type @ addNativeButton [got"..type(text).." '"..text.."'']")
    window.items[id].text = text
end
...
function getNativeButtonIcon(id)
    if not id then return end
    assert(tonumber(id),"Bad argument @ getNativeButtonIcon [expected number at argument 1,  got "..type(id).." '"..id.."'']")
    --assert(icon ~= "box","Invalid icon type @ addNativeButton [got"..type(icon).." '"..icon.."'']")
    return window.items[id] and window.items[id].icon
end

function setNativeButtonPrice(id,price) 
    if not id or not price then return end
    assert(tonumber(id),"Bad argument @ setNativeButtonPrice [expected number at argument 1,  got "..type(id).." '"..id.."'']")
    assert(price ~= "box","Invalid icon type @ addNativeButton [got"..type(price).." '"..price.."'']")
    window.items[id].price = price
end

function getNativeButtonPrice(id)
    if not id then return end
    assert(tonumber(id),"Bad argument @ getNativeButtonPrice [expected number at argument 1,  got "..type(id).." '"..id.."'']")
    return window.items[id].price
end
function getNativeButtonName(id)
    if not id then return end
    assert(tonumber(id),"Bad argument @ getNativeButtonName [expected number at argument 1,  got "..type(id).." '"..id.."'']")
    return window.items[id].text
end

function removeNativeButtonIcon(id)
    if not id then return end
    assert(tonumber(id),"Bad argument @ removeNativeButtonIcon [expected number at argument 1,  got "..type(id).." '"..id.."'']")
...
function mapCore ()
	...
	if localPlayer then
	local txd = engineLoadTXD ("files/models/ammu_nation_ls.txd", 4552)
	engineImportTXD(txd, 4552)
	local dff = engineLoadDFF ("files/models/ammu_nation_ls.dff", 4552)
	engineReplaceModel(dff, 4552, true)
	local col = engineLoadCOL("files/models/ammu_nation_ls.col")
...
function mapping ()
	if not localPlayer then
	local x,y,z = 1381.5068359375, -1282.3505859375, 13.546875
	local p = createPed (179, x,y,z, 90)
	...
	weaponname = {}
		...
		local t = prateleiras[i]
			for k=1, #t do
			local tabela = t[k]
			local arma = tabela.name
			local d = exports['PUNK+Armas']:getData()[arma]
				if d then
				local x,y,z = unpack (tabela['position'].default)
				local px,py,pz = unpack (tabela['position'].active)
				local rx,ry,rz = unpack (tabela['rotation'].default)
				local rx1,ry1,rz1 = unpack (tabela['rotation'].active)
				local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
		...
		local d = exports['PUNK+Armas']:getData()[arma]
			if d then
			local x,y,z = 1379, -1284.25, 13.325
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1378.75, -1284.25, 13.325
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1379.25, -1284.25, 13.325
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1379.125, -1284.125, 13.325
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1378.875, -1284.125, 13.325
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
		...
		local d = exports['PUNK+Armas']:getData()[arma]
			if d then
			local x,y,z = 1379.75, -1284.25, 13.33
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1379.50, -1284.25, 13.33
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1380, -1284.25, 13.33
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1379.625, -1284.125, 13.33
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
			
			local x,y,z = 1379.875, -1284.125, 13.33
			local rx,ry,rz = 0,0,-90
			local ob = exports['PUNK+Objetos']:createObject (arma, x,y,z, rx,ry,rz)
			setElementCollisionsEnabled(ob, false)
		...
		triggerClientEvent (source, 'weapon_shop', source, prateleiras)
		end)
	end
	
	if localPlayer then
	
	addEvent ('weapon_shop', true)
	addEventHandler ('weapon_shop', getRootElement(), function (p)
	prateleiras = p
	end)
...
function obterArmas (jogador)
	if jogador then return minhasArmas[jogador] end
return minhasArmas
end

function hasWeapon (w)
local properties, projectiles, armas = getData ()
	if armas[localPlayer] then
		for slot, t in pairs (armas[localPlayer]) do
			for arma, tabela in pairs (t) do
				if arma == w then return true end
			end
		end
	end
end
	...
	counter = 0
		...
		addEventHandler ("PUNKArmasLoja", getRootElement(), chegou)
		...
		function loja ()
			...
			aberta = true
			destroyNativeUI ()
			createNativeUI(" ", "Weapons",  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
			counter = 0
			--Setup
			local tabela = prateleiras[1][21] or prateleiras[1][1]
				if minhasArmas[tabela.name] then
				icone = 'shop_gunclub_icon_b'
				else
				icone = false
				end
				if weapons[tabela.name] and weapons[tabela.name].price then price = weapons[tabela.name].price else price = 350 end
			idBotao = addNativeButton(tabela.name, tocolor (255,255,255), icone, price)
			local px,py,pz = unpack (tabela['position'].active)
			local rx1,ry1,rz1 = unpack (tabela['rotation'].active)
			local ob = tabela.object
			objeto = ob
			currentWeapon = tabela.name
			updateRenderStats ()
			...
			--addEventHandler ("onClientChangeButton", getRootElement(), mudar
				...
				if minhasArmas then
					if minhasArmas[currentWeapon] then
						if minhasArmas[currentWeapon].upgrades then
							for item, valor in pairs (minhasArmas[currentWeapon].upgrades) do
								if valor == true then
								applyWeaponUpgrade (objeto, item)
								end
							end
						end
					end
			...
			setCameraTarget (localPlayer)
				...
				local t = prateleiras[i]
					for k=1, #t do
					local tabela = t[k]
						if objeto == tabela.object then
						local x,y,z = unpack (tabela['position'].default)
						local px,py,pz = unpack (tabela['position'].active)
						local rx,ry,rz = unpack (tabela['rotation'].default)
						local rx1,ry1,rz1 = unpack (tabela['rotation'].active)
						local ob = tabela.object
						moveObject (ob, 100, x,y,z, -rx1,-ry1,-rz1, 'Linear')
						break
						end
					end
				end
				...
				if estado then
					...
					elseif botao == "escape" or botao == "backspace" then
						...
						cancelEvent ()
							...
							createNativeUI(" ", "Weapons",  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", false ,10,"left")
								for i=1, #prateleiras do
									for k=1, #prateleiras[i] do
									local tab = prateleiras[i][k]
										if objeto == tab.object then
										tabela = tab
										break
										end
									end
								...
								if tabela.price then price = tabela.price else price = 350 end
							idBotao = addNativeButton(tabela.name, tocolor (255,255,255), icone, price)
							playNativeSound ('return')
							local x,y,z = unpack (tabela.camera['default']['position'])
							local px,py,pz = unpack (tabela.camera['default']['target'])
					...
					elseif botao == 'd' or botao == 'arrow_r' then
					cancelEvent ()
						if active == true then return end
					mudar (true)
					playNativeSound ("change")
					elseif botao == 'a' or botao == 'arrow_l' then
					cancelEvent ()
						if active == true then return end
					mudar ()
					playNativeSound ("change")
					elseif botao == 'w' or botao == 'arrow_u' then
					cancelEvent ()
						if active == true then return end
					trocar (true)
					playNativeSound ("change")
					elseif botao == 's' or botao == 'arrow_d' then
					cancelEvent ()
						if active == true then return end
					trocar ()
					playNativeSound ("change")
		...
		tt = nil
			...
			local px,py,pz = unpack (tabela.camera['default']['target'])
			setCameraMatrix (x,y,z, px,py,pz)
			setNativeButtonName (idBotao, tabela.name)
			setNativeButtonPrice (idBotao, tabela.price)
			currentWeapon = tabela.name
			updateRenderStats ()
				if minhasArmas[tabela.name] then
				setNativeButtonIcon (idBotao, 'shop_gunclub_icon_b')
				else
				removeNativeButtonIcon (idBotao)
				end
			
			end
		end
		
		function trocar (estado)
		retornarArma ()
		tt = nil
			for i=1, #prateleiras do
			local t = prateleiras[i]
				for k=1, #prateleiras[i] do
				local tabela = prateleiras[i][k]
					if objeto == tabela.object then
						if estado then
							if prateleiras[i+1][k] then
							tt = prateleiras[i+1][k]
							else
							tt = prateleiras[i+1][1]
							end
						else
							if prateleiras[i-1][k] then
							tt = prateleiras[i-1][k]
							else
							tt = prateleiras[i-1][#prateleiras[i-1]]
							end
						end
					break
					end
		...
		function comprar (id, text)
			for i=1, #prateleiras do
				for k=1, #prateleiras[i] do
				local tab = prateleiras[i][k]
					if text == tab.name then
					tabela = tab
					break
					end
				end
			end
			if minhasArmas[text] then
			destroyNativeUI ()
			local ttt = getWeaponUpgrades (text)
			createNativeUI(" ", text..' UPGRADES',  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", #ttt ,10,"left")
				
				if hasUpgrade (text, 'Extended Clip') == true then
				local stats = getUpgradeStat (text, 'Extended Clip', 'clip')
					if stats then
					rounds = stats
					end
				else
				local data = getData ()
					if data[text] then
					rounds = data[text].clip
					end
				end
			idBotao = addNativeButton('Rounds x '..rounds, tocolor (255,255,255), false, rounds)
			local t = weapons[text].upgrades
				if t then
					for i=1, #t do
					local tempButton = addNativeButton(t[i].name, tocolor (255,255,255), false, t[i].price)
					local upgrade = t[i].name
						if hasUpgrade (text, upgrade) == true then
							if isElement (objeto) then
							--applyWeaponUpgrade (objeto, tab)
							end
						setNativeButtonIcon (tempButton, 'shop_gunclub_icon_a')
						elseif hasUpgrade (text, upgrade) == false then
						setNativeButtonIcon (tempButton, 'accept3')
						end
					end
				end
				
			local x,y,z = unpack (tabela.camera['active']['position'])
			local px,py,pz = unpack (tabela.camera['active']['target'])
			setCameraMatrix (x,y,z, px,py,pz)
			active = true
			currentWeapon = text
			updateRenderStats ()
				...
				local preco = getNativeButtonPrice (id) or 350
				if active then
					if string.find (text, 'Rounds') then
					local a = string.gsub (text, 'Rounds x ', '')
					triggerServerEvent ("PUNKArmasComprar", localPlayer, localPlayer, currentWeapon, preco, tonumber (a))
					else
						if hasUpgrade (currentWeapon, text) == true then
						tempDescription = {'Item already equipped'}
						else
							if hasUpgrade (currentWeapon, text) == false then
							tempDescription = {'Item equipped.'}
							else
							local preco = getNativeButtonPrice (id) or 350
								if getPlayerMoney (localPlayer) >= preco then
								tempDescription = {'Item purchased.'}
								else
								tempDescription = {"Insuficient funds $"..preco}
									setTimer (function ()
									tempDescription = nil
									end, 1000, 1)
								return
						...
						triggerServerEvent ('applyUpgradeForPlayer', localPlayer, localPlayer, currentWeapon, text)
						--applyWeaponUpgrade (objeto, text)
						local stat = getUpgradeStat (currentWeapon, text, 'clip')
							if stat then
							setNativeButtonName (idBotao, 'Rounds x '..stat)
							end
						setNativeButtonIcon (id, 'shop_gunclub_icon_b')
						local tab = weapons[currentWeapon].upgrades
							for i=1, #tab do
								if tab[i].name == text then
									if tab[i]['remove'] then
									local tab = tab[i]['remove']
										for i=1, #tab do
										local t = tab[i]
											for k, tabela in pairs (window.items) do
												if tabela.text then
													if tabela.text == t then
														if hasUpgrade (currentWeapon, tabela.text) == true then
														setNativeButtonIcon (k, 'accept3')
														end
													end
												end
											end
											if minhasArmas[currentWeapon] then
												if minhasArmas[currentWeapon].upgrades then
												local a = minhasArmas[currentWeapon].upgrades
													for item, valor in pairs (a) do
														if item == t then
														a[item] = false
														end
													end
					...
					if getPlayerMoney (localPlayer) >= preco then
					minhasArmas[text] = {}
					triggerServerEvent ("PUNKArmasComprar", localPlayer, localPlayer, text, preco)
					setNativeButtonIcon (id, "shop_gunclub_icon_b")
					...
					createNativeUI(" ", text..' UPGRADES',  'assets/ammu-nation.png', tocolor(0,0,0), tocolor(255,255,255), tocolor(255, 255, 255), "left", #ttt ,10,"left")
					 ...
					 idBotao = addNativeButton('Rounds x '..rounds, tocolor (255,255,255), false, weapons[text] and weapons[text].price or 350)
					local t = weapons[text] and weapons[text].upgrades
						if t then
							for i=1, #t do
							local tempButton = addNativeButton(t[i].name, tocolor (255,255,255), false, t[i].price)
							local upgrade = t[i].name
								if upgrade == 'Default Clip' or upgrade == 'Tint 1' then
									if isElement (objeto) then
										if upgrade == 'Default Clip' then
										applyWeaponUpgrade (objeto, upgrade)
										end
									end
								setNativeButtonIcon (tempButton, 'shop_gunclub_icon_a')
								end
	...
	local marker = createMarker (1379.9541015625, -1282.4208984375, 13.546875, "cylinder", 1, 170,0,0,0)
		...
		local j = getElementsByType ('player')[1]
				setTimer (function ()
				triggerClientEvent (j, "PUNKArmasLoja", j, true)
				end, 1000, 1)
				
		function comprar (jogador, arma, preco, rounds)
			if getPlayerMoney (jogador) >= preco then
			local data = getData ()
				if data then
					if not rounds then
					rounds = data[arma].clip
					end
				takePlayerMoney (jogador, preco)
					if not minhasArmas[jogador] then minhasArmas[jogador] = {} end
					
					giveWeapon (jogador, arma, rounds, true)
				
					if not minhasArmas[jogador][arma] then
					minhasArmas[jogador][arma] = {rounds = rounds}
					applyUpgradeForPlayer(jogador, arma, 'Tint 1')
					applyUpgradeForPlayer(jogador, arma, 'Default Clip')
					else
					minhasArmas[jogador][arma].rounds = minhasArmas[jogador][arma].rounds or 0 + rounds
					end
				rounds = nil
				local arma, ob = getPedWeapon (jogador)
					if isElement (ob) then
					setElementAlpha (ob, getElementAlpha (jogador))
					end
				end
			end
		end
		addEvent ("PUNKArmasComprar", true)
		addEventHandler ("PUNKArmasComprar", getRootElement (), comprar)
...
shaders = {}
	...
	if weapons then
		for arma, tabela in pairs (weapons) do
		local tabela = tabela.upgrades
			for j=1, #tabela do
			local tab = tabela[j]
				if tab['tex'] then
				local upgrade = tab.name
					if not shaders[arma] then shaders[arma] = {} end
				shaders[arma][upgrade] = {}
				local tempTab = tab.tex
					for j=1, #tempTab do
					shaders[arma][upgrade][j] = {}
					shaders[arma][upgrade][j].tex = tempTab[j].name
					shaders[arma][upgrade][j].stat = tempTab[j].name
						if tempTab[j].file then
						local texture = dxCreateTexture(tempTab[j].file)
						local shader = dxCreateShader(dadosShader, 1, 0, false, 'object')
						dxSetShaderValue(shader, "tex",  texture)
						shaders[arma][upgrade][j].element = shader
						end
					end
				end
			end
	...
	function applyUpgradeForPlayer(jogador, weapon, upgrade)
		--if not minhasArmas[jogador] then minhasArmas[jogador] = {} end
		if minhasArmas[jogador] then
			if minhasArmas[jogador][weapon] then
				if not minhasArmas[jogador][weapon].upgrades then minhasArmas[jogador][weapon].upgrades = {} end
				if not minhasArmas[jogador][weapon].upgrades[upgrade] then minhasArmas[jogador][weapon].upgrades[upgrade] = true end
			local tab = weapons[weapon] and weapons[weapon].upgrades
				if tab then
					for i=1, #tab do
						if tab[i].name == upgrade then
							if tab[i]['remove'] then
							local tab = tab[i]['remove']
								for i=1, #tab do
								local t = tab[i]
									if hasPlayerUpgrade (jogador, weapon, t) == true then
									minhasArmas[jogador][weapon].upgrades[t] = false
									else
									minhasArmas[jogador][weapon].upgrades[t] = nil
									end
								end
							end
						end
		...
		if getPedWeapon(jogador) == weapon then
			if not localPlayer then
			triggerClientEvent ('updateMyWeapons', jogador, minhasArmas[jogador])
			end
		end
	end
	addEvent ('applyUpgradeForPlayer', true)
	addEventHandler ('applyUpgradeForPlayer', getRootElement(), applyUpgradeForPlayer)
	
	function takeUpgradeFromPlayer(jogador, weapon, upgrade)
		if minhasArmas[jogador] then
			if minhasArmas[jogador][weapon] then
				if not minhasArmas[jogador][weapon].upgrades then minhasArmas[jogador][weapon].upgrades = {} end
				if not minhasArmas[jogador][weapon].upgrades[upgrade] then minhasArmas[jogador][weapon].upgrades[upgrade] = false end
			end
		end
		if getPedWeapon(jogador) == weapon then
			if not localPlayer then
	...
	addEvent ('takeUpgradeFromPlayer', true)
	addEventHandler ('takeUpgradeFromPlayer', getRootElement(), takeUpgradeFromPlayer)

shaderID = {}
	function applyWeaponUpgrade (weapon, upgrade)
	local nome = exports['PUNK+Armas']:getWeaponName (weapon)
		if not nome then
		nome = currentWeapon 
		end
		if shaders[nome] then
			if shaders[nome][upgrade] then
			local tab = shaders[nome][upgrade]
				for i=1, #tab do
				local tex = tab[i].tex
				local shader = tab[i].element
					if shader then
					engineApplyShaderToWorldTexture (shader, tex, weapon)
					end
				end
			end
			
		local tab = weapons[nome].upgrades
			for i=1, #tab do
				if tab[i].name == upgrade then
					if tab[i]['remove'] then
					local tab = tab[i]['remove']
	...
	local weapon = exports['PUNK+Armas']:getDummy (weapon)
		if isElement (weapon) then
		local stats = getUpgradeStats (nome, upgrade)
			if stats then
				for stat, value in pairs (stats) do
				local setou = exports['PUNK+Armas']:setCustomWeaponProperty (weapon, stat, value)
				end
			end
		end
	end	

	function removeWeaponUpgrade (weapon, upgrade)
	local nome = exports['PUNK+Armas']:getWeaponName (weapon)
		if not nome then nome = currentWeapon end
		
		if shaders[nome] then
			if shaders[nome][upgrade] then
			local t = shaders[nome][upgrade]
				for i=1, #t do
				local tex = t[i].tex
				local shader = t[i].element
					if shader then
					engineRemoveShaderFromWorldTexture (shader, tex, weapon)
					end
				end
			end
		end
	...
	function updateMy (w)
		if source == localPlayer and w then minhasArmas = w end
	local jogador = source
	
		if w then
			for weapon, tabela in pairs (w) do
			local a, ob, d = getPedWeapon (jogador)
				if a == weapon then
					if tabela.upgrades then
						for upgrade, valor in pairs (tabela.upgrades) do
						local stats = getUpgradeStats (weapon, upgrade)
							if stats then
								for stat, value in pairs (stats) do
								--local setou = exports['PUNK+Armas']:setCustomWeaponProperty (d, stat, value)
								--local setou = exports['PUNK+Armas']:setCustomWeaponProperty (ob, stat, value)
								end
							end
							if valor == true then
							applyWeaponUpgrade (d, upgrade)
							else
							--removeWeaponUpgrade (d, upgrade)
							end
						end
					end
				break
	...
	addEvent ('updateMyWeapons', true)
	addEventHandler ('updateMyWeapons', getRootElement(), updateMy)

	addEventHandler ('onPlayerWeaponSwitch', getRootElement (), function (anterior, arma)
	--triggerClientEvent ('updateMyWeapons', source, minhasArmas[source])
	end)
	
function updateUpgrade (atual, net)
local arma = currentWeapon
	if arma then
	local icone = getNativeButtonIcon (atual)
		if icone == 'shop_gunclub_icon_a' then
		setNativeButtonIcon (atual, 'shop_gunclub_icon_b')
		elseif icone == 'shop_gunclub_icon_b' then
		setNativeButtonIcon (atual, 'shop_gunclub_icon_a')
		end
		if icone == 'accept3' then
		setNativeButtonIcon (atual, 'accept4')
		elseif icone == 'accept4' then
		setNativeButtonIcon (atual, 'accept3')
		end
		if currentUpgrade then
		local icone = getNativeButtonIcon (currentButton)
			if icone == 'shop_gunclub_icon_a' then
			setNativeButtonIcon (currentButton, 'shop_gunclub_icon_b')
			elseif icone == 'shop_gunclub_icon_b' then
			setNativeButtonIcon (currentButton, 'shop_gunclub_icon_a')
			end
			if icone == 'accept3' then
			setNativeButtonIcon (currentButton, 'accept4')
			elseif icone == 'accept4' then
			setNativeButtonIcon (currentButton, 'accept3')
			end
		end
	local u = getWeaponUpgrades (arma)
		for j=1, #u do
		local upgrade = u[j]
			if upgrade and net == upgrade or string.find (net, 'Rounds') then
				if currentUpgrade then
					if hasUpgrade (arma, currentUpgrade) ~= true then
					removeWeaponUpgrade (objeto, currentUpgrade)
					end
				end
				local tab = weapons[arma].upgrades
					found = nil
						for i=1, #tab do
							if found ~= true and tab[i].name == currentUpgrade then
							found = true
								if tab[i]['remove'] then
								local tab = tab[i]['remove']
									for i=1, #tab do
									local t = tab[i]
										if hasUpgrade (arma, t) == true then
										applyWeaponUpgrade (objeto, t)
										end
									end
								end
							end
			...
			currentButton = atual
			updateRenderStats ()
			applyWeaponUpgrade (objeto, currentUpgrade)
			break
			end
		end
...
addEventHandler ('onClientChangeButton', getRootElement(), updateUpgrade)
...
function saveIni ()
	...
	function salvarArmas (jogador)
		if minhasArmas[jogador] then
		local conta = getPlayerAccount (jogador)
			for arma, tabela in pairs (minhasArmas[jogador]) do
			--minhasArmas[jogador][arma].rounds = getPedTotalAmmo(jogador, arma)
			end
		setAccountData (conta, 'PUNKArmas', nil)--toJSON (minhasArmas[jogador]))
		end
	end

	function carregarArmas (jogador)
	local conta = getPlayerAccount (jogador)
		if getAccountData (conta, 'PUNKArmas') then
		minhasArmas[jogador] = fromJSON (getAccountData (conta, 'PUNKArmas'))
			for arma, tabela in pairs (minhasArmas[jogador]) do
			local slot = getSlotFromWeapon (arma)
				if getPedWeapon (jogador, slot) == arma then
				--takeWeapon (jogador, arma)
				end
			giveWeapon (jogador, arma, tabela.rounds, true)
			end
		triggerClientEvent ('updateMyWeapons', jogador, minhasArmas[jogador])
		end
	end
